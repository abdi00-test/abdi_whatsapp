...
def extract_instagram_post_info_with_ytdlp(url: str) -> Dict[str, str]:
    ...
    try:
        ...
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            ...
            media_count = len(info.get('entries', [])) if info.get('entries') else (1 if info.get('url') else 0)
            is_carousel = media_count > 1
...
def download_instagram_post_with_ytdlp(url: str) -> list:
    ...
    try:
        ...
        logger.error(f"Failed to download Instagram post with yt-dlp: {e}")
        raise
...
def handle_link(recipient_id: str, url: str):
    ...
    # Handle Instagram reel specifically
    if platform == 'instagram_reel':
        handle_instagram_reel(recipient_id, url)
        return
    
    # Handle Instagram post (including carousel)
    if platform == 'instagram_post':
        handle_instagram_post(recipient_id, url)
        return
...
def handle_instagram_post(recipient_id: str, url: str):
    """Handle Instagram post download and send (including carousel posts)"""
    try:
        # Send processing message
        messenger.send_message("📥 *Downloading Instagram post...*", recipient_id)
        
        # Extract post information using yt-dlp
        post_info = extract_instagram_post_info_with_ytdlp(url)
        title = post_info['title']
        creator = post_info['creator']
        is_carousel = post_info['is_carousel']
        media_count = post_info['media_count']
        
        # Send info message with post details
        if is_carousel:
            info_message = f"🖼️ *{title}*\n👤 *Creator: {creator}*\n📊 *Media Count: {media_count} items*"
        else:
            info_message = f"🖼️ *{title}*\n👤 *Creator: {creator}*"
        messenger.send_message(info_message, recipient_id)
        
        # Download the post using yt-dlp
        if is_carousel:
            messenger.send_message("⬇️ *Downloading carousel media files...*", recipient_id)
        else:
            messenger.send_message("⬇️ *Downloading media file...*", recipient_id)
            
        file_paths = download_instagram_post_with_ytdlp(url)
        ...
        logger.error(f"Instagram post handling failed: {e}")
        messenger.send_message(f"❌ *Failed to download Instagram post*\nError: {str(e)}", recipient_id)
